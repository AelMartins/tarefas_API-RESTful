...
app.put('/concluirTarefa/:id', (req, res) => {
    ...
    if (!tarefaEncontrada) {
        return res.status(404).send(`TAREFA COM ID " ${id} " NÃO FOI ENCONTRADA!`);
    }
...
app.put('/atualizarTarefa', (req, res) => {
    ...
    if (!atualizarTarefa) {
        return res.status(404).send(`A TAREFA COM ID " ${id} " NÃO FOI ENCONTRADA!`);
    }
...
app.delete('/deletarTarefa', (req, res) => {
    const { id } = req.body;
    const deletaTarefa = tarefas.find(callbackTarefa => callbackTarefa.id === id);
    if (!deletaTarefa) {
        return res.status(404).send(`A TAREFA COM ID " ${id} " NÃO FOI ENCONTRADA!`);
    }
    tarefas.splice(id);
    let printHTML = `TAREFA DELETADA COM SUCESSO! `
    const tarefasString = JSON.stringify(tarefas, null, 2);
    printHTML += `SEGUE A LISTA DE TODAS AS SUAS TAREFAS AINDA EXISTENTES: <pre>${tarefasString}</pre>`;
    res.send(printHTML);
});